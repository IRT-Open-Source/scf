...
<schema xmlns="http://purl.oclc.org/dsdl/schematron"  queryBinding="xslt" schemaVersion="ISO19757-3">
    ...
    <title>Testing doubleHeight parameter with 'default'</title>
    <pattern id="doubleHeight">
        ...
        <rule context="StlXml/BODY/TTICONTAINER/TTI[position() = 1]/TF">
            <assert test="name(child::*[name(.) != 'space'][1]) = 'StartBox'"> 
                Expected element at position one (space elements are ignored): "StartBox" Value from test: "<value-of select="name(child::*[name(.) != 'space'][1])"/>"
            </assert>
        </rule>
    </pattern>            
</schema>
