...
<schema xmlns="http://purl.oclc.org/dsdl/schematron"  queryBinding="xslt" schemaVersion="ISO19757-3">
    ...
    <title>Testing referenced tt:style elements by tt:span elements without changing foreground and background</title>
    <pattern id="DoubleheightLinebreak">
        ...
        <rule context="StlXml/BODY/TTICONTAINER/TTI[position() = 1]/TF">
            ...
            <assert test="name(child::*[name(.) != 'space'][3]) = 'EndBox'"> 
                Expected value at position 3 (space elements are ignored): "EndBox". Value from test: "<value-of select="name(child::*[name(.) != 'space'][3])"/>" 
            </assert>
            <assert test="name(child::*[name(.) != 'space'][4]) = 'EndBox'"> 
                Expected value at position 4 (space elements are ignored): "EndBox". Value from test: "<value-of select="name(child::*[name(.) != 'space'][4])"/>" 
            </assert>
        </rule>
    </pattern>            
</schema>
