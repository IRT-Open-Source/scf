...
<schema xmlns="http://purl.oclc.org/dsdl/schematron"
    ...
    <title>Testing tts:color and tts:background with value AlphaMagenta</title>
    <pattern id="SpanIDAttribute">
        ...
        <rule context="tt:tt/tt:head/tt:styling/tt:style[@xml:id = 'AlphaMagentaOnAlphaWhite']">
            <assert test="@tts:backgroundColor = '#ffffffff'">
                Expected value: "#ffffffff" Value from test: "<value-of select="@tts:backgroundColor"/>"  
            </assert> 
            <assert test="@tts:color = '#ff00ffff'">
                Expected value: "#ff00ffff" Value from test: "<value-of select="@tts:color"/>"
            </assert> 
        </rule>
        <rule context="tt:tt/tt:head/tt:styling/tt:style[@xml:id = 'AlphaWhiteOnAlphaMagenta']">
            <assert test="@tts:backgroundColor = '#ff00ffff'">
                Expected value: "#ff00ffff" Value from test: "<value-of select="@tts:backgroundColor"/>"
            </assert> 
