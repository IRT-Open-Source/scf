...
<schema xmlns="http://purl.oclc.org/dsdl/schematron"
    ...
    <title>Testing tts:color and tts:background with value AlphaYellow</title>
    <pattern id="SpanIDAttribute">
        ...
        <rule context="tt:tt/tt:head/tt:styling/tt:style[@xml:id = 'AlphaWhiteOnAlphaYellow']">
            <assert test="@tts:backgroundColor = '#ffff00ff'">
                Expected value: "#ffff00ff" Value from test: "<value-of select="@tts:backgroundColor"/>"
             </assert> 
            <assert test="@tts:color = '#ffffffff'">
                Expected value: "#ffffffff" Value from test: "<value-of select="@tts:color"/>"   
            </assert> 
        </rule>
        <rule context="tt:tt/tt:head/tt:styling/tt:style[@xml:id = 'AlphaYellowOnAlphaWhite']">
            <assert test="@tts:backgroundColor = '#ffffffff'">
                Expected value: "#ffffffff" Value from test: "<value-of select="@tts:backgroundColor"/>"   
            </assert> 
            <assert test="@tts:color = '#ffff00ff'">
                Expected value: "#ffff00ff" Value from test: "<value-of select="@tts:color"/>"   
            </assert> 
        </rule>
    </pattern>            
</schema>
