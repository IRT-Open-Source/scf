...
<schema xmlns="http://purl.oclc.org/dsdl/schematron"
    ...
    <title>Testing tts:color and tts:background with value AlphaBlack</title>
    <pattern id="SpanIDAttribute">
        ...
        <rule context="tt:tt/tt:head/tt:styling/tt:style[@xml:id = 'AlphaBlackOnAlphaWhite']">
            <assert test="@tts:backgroundColor = '#ffffffff'">
                Expected value: "#ffffffff" Value from test: "<value-of select="@tts:backgroundColor"/>"
            </assert> 
            <assert test="@tts:color = '#000000ff'">
                Expected value: "#000000ff" Value from test: "<value-of select="@tts:color"/>"
            </assert> 
        </rule>
        <rule context="tt:tt/tt:head/tt:styling/tt:style[@xml:id = 'AlphaWhiteOnAlphaBlack']">
            <assert test="@tts:backgroundColor = '#000000ff'">
                Expected value: "#000000ff" Value from test: "<value-of select="@tts:backgroundColor"/>"
            </assert> 
