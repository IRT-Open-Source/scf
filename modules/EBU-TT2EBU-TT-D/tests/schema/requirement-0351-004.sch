...
<schema xmlns="http://purl.oclc.org/dsdl/schematron"
    ...
    <title>Testing tts:color and tts:background with value AlphaGreen</title>
    <pattern id="SpanIDAttribute">
        ...
        <rule context="tt:tt/tt:head/tt:styling/tt:style[@xml:id = 'AlphaGreenOnAlphaWhite']">
            <assert test="@tts:backgroundColor = '#ffffffff'">
                Expected value: "#ffffffff" Value from test: "<value-of select="@tts:backgroundColor"/>"  
            </assert> 
            <assert test="@tts:color = '#008000ff'">
                Expected value: "#008000ff" Value from test: "<value-of select="@tts:color"/>"  
            </assert> 
        </rule>
        <rule context="tt:tt/tt:head/tt:styling/tt:style[@xml:id = 'AlphaWhiteOnAlphaGreen']">
            <assert test="@tts:backgroundColor = '#008000ff'">
                Expected value: "#008000ff" Value from test: "<value-of select="@tts:backgroundColor"/>"  
            </assert> 
