...
<schema xmlns="http://purl.oclc.org/dsdl/schematron"
    ...
    <title>Testing NewBackground element with unchanging AlphaBlack</title>
    <pattern id="FirstTimeStamps">
        ...
        <rule context="tt:tt/tt:body/tt:div/tt:p">
            <assert test="count(tt:span) eq 1">
                Expected value: "1" Value from test: "<value-of select="count(tt:span)"/>"
            </assert>
        </rule>
        <rule context="tt:tt/tt:body/tt:div/tt:p/tt:span">
            <assert test="@style = 'AlphaWhiteOnAlphaBlack'">
                Expected value: "AlphaWhiteOnAlphaBlack" Value from test: "<value-of select="@style"/>"
            </assert>
        </rule>
    </pattern>            
</schema>
